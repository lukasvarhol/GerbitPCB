...
export default function App() {
	...
	const payload = {
	    ...
	    items: selectedComponents.map(c => ({
		supplier: c.supplier,
		sku: c.sku,
		quantity: c.qty,
		unitPrice: c.priceEur,
	    })),
	};

	try {
	    const response = await fetch('http://localhost:8090/api/transactions', {
		method: 'POST',
		...
		alert(`Order placed successfully! ID: ${data.transactionId}`);
	    } else {
		alert(`Order failed with status: ${data.status}`);
	    }
	} catch (err) {
	    alert('Could not reach the broker...');
	}
    };
